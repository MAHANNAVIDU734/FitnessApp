...
class FirestoreExcerciseManager {
     ...
     */
   private func storeSignedUpUserDetailsOnFirestoreDb(completionWithPayload:CompletionHandlerWithData?){
    ...
    /**
     This is used to  fetch the exercise data  stored on  Firestore Db     */
    func getExerciseDataStoredOnFirestoreDb(completionWithPayload:CompletionHandlerWithData?){
        let exercisesDocumentRef = Firestore.firestore().collection(FirestoreCollections.excercise.rawValue)
        exercisesDocumentRef.getDocuments() { (querySnapshot, err) in
            if let err = err {
                completionWithPayload?(false,err.localizedDescription,nil)
                print("Error getting documents: \(err)")
            } else {
                var exercisesArray = [FirestoreExcercise]()
                for document in querySnapshot!.documents {
                    let _exerciseDataDictoanary = document.data()
                    var exerciseData =  FirestoreExcercise(dictionary: _exerciseDataDictoanary)
                    if let _exerciseData = exerciseData {
                        exercisesArray.append(_exerciseData)
                    }
                }
                completionWithPayload?(true,nil,exercisesArray)
            }
        }
    }
}
